...
// scripts/audit-theme.ts
var import_playwright_core = require("playwright-core");
var fs = __toESM(require("fs"));
var path = __toESM(require("path"));
var BASE = (process.env.APERIO_BASE_URL ?? "http://localhost:3000").replace(/\/$/, "");
var SUFFIX = process.env.APERIO_REPORT_SUFFIX ?? "run";
var SHOTS = process.env.APERIO_SHOTS_DIR ?? path.join(process.env.TEMP ?? "C:/Windows/Temp", "opencode", "theme-shots");
{
  const host = new URL(BASE).hostname.toLowerCase();
  if (!(host === "localhost" || host === "127.0.0.1" || host === "::1")) {
    console.error(`[audit-theme] REFUS : la cible \xAB ${BASE} \xBB n'est pas locale (h\xF4te="${host}").`);
    process.exit(2);
  }
}
var BUYER = { email: "buyer@aperio.gallery", password: "buyer123" };
var PHOTOGRAPHER = { email: "photographer@aperio.gallery", password: "photo123" };
var PHOTO_SLUG = "close-up-of-tree-bark-with-resin-droplets";
var PUBLIC_ROUTES = [
  "/",
  "/about",
  "/help",
  ...
  "/pricing",
  "/photos",
  "/photos?category=madagascar",
  "/prints",
  "/prints?category=malagasy-art",
  "/login",
  "/register",
  `/photo/${PHOTO_SLUG}`
];
var BUYER_ROUTES = ["/profile", "/checkout", "/wallet", "/purchases", "/collections"];
var PHOTOGRAPHER_ROUTES = ["/dashboard", "/payout"];
...
var rows = [];
var log = (s) => console.log(`[audit-theme] ${s}`);
...
function shotName(route, theme, vp, extra = "") {
  const safe = (route === "/" ? "home" : route).replace(/[^\w-]+/g, "-").replace(/^-+|-+$/g, "");
  return `${safe}__${theme}__${vp}${extra ? "__" + extra : ""}.png`;
}
async function scanRoute(page, route, theme, vp, auth) {
  const url = `${BASE}${route}`;
  await page.goto(url, { waitUntil: "load", timeout: 9e4 });
  await page.waitForTimeout(650);
  const scan = await page.evaluate(WCAG_SCANNER);
  ...
  if (failures.length > 0) {
    ...
    log(`  \u2713 ${route.padEnd(42)} ${theme.padEnd(5)} ${vp.name.padEnd(7)} \u2192 0 (${scan.media} \xE9l\xE9ments sur image ignor\xE9s pour la statique)`);
  }
...
async function main() {
  log(`Cible : ${BASE} (contr\xF4l\xE9e locale).`);
  ...
  const browser = await import_playwright_core.chromium.launch({ channel: "chrome", headless: true }).catch((err) => {
    console.error(
      "[audit-theme] Impossible de lancer Chrome canal local (" + err.message + ').\nAucun navigateur ne sera t\xE9l\xE9charg\xE9 : utilise un Chrome install\xE9 (channel: "chrome").'
    );
    process.exit(3);
  });
  try {
    ...
    log("Test TOGGLE (un seul clic r\xE9el : clair \u2192 sombre)\u2026");
    {
      ...
      await page.goto(`${BASE}/`, { waitUntil: "load", timeout: 9e4 });
      const attrAtFirstPaint = await page.evaluate(() => document.documentElement.getAttribute("data-theme"));
      const beforeScan = await page.evaluate(WCAG_SCANNER);
      const t = await toggleCheck(page);
      log(`  attribut data-theme au 1er rendu : ${attrAtFirstPaint} (attendu light)`);
      log(`  avant : data-theme=${t.beforeAttr} body=${t.beforeBody}  \u2192  apr\xE8s : data-theme=${t.afterAttr} body=${t.afterBody}`);
      log(`  ic\xF4ne/aria du bouton synchronis\xE9e : ${t.labelChanged}`);
      if (t.ok && attrAtFirstPaint === "light" && beforeScan.theme === "light") {
        ...
        console.error("[audit-theme] \u2717 \xC9CHEC du test bouton : le clic n'a pas (re)bascul\xE9 le th\xE8me.");
        process.exitCode = 1;
      }
    ...
    for (const theme of onlyThemes.length === 1 && onlyThemes[0] === "none" ? [] : THEMES.filter((t) => onlyThemes.includes(t))) {
      for (const vp of VIEWPORTS) {
        log(`--- Th\xE8me \xAB ${theme} \xBB, ${vp.name} (${vp.width}\xD7${vp.height}) ---`);
        for (const route of PUBLIC_ROUTES) {
          ...
          try {
            ...
            log(`  \u2717 ${route} ERREUR : ${err.message}`);
            rows.push({ route, theme, viewport: vp.name, width: vp.width, ok: false, failures: -1, media: -1, worst: null });
          }
          await page.close();
          await ctx.close();
        }
  ...
  stdout.push(`SYNTH\xC8SE \u2014 routes \xD7 th\xE8mes \xD7 tailles : ${rows.length} | OK ${okCount} | KO ${bad.length} | \xE9chec total : ${bad.reduce((s, r) => s + Math.max(r.failures, 0), 0)}`);
  stdout.push("=".repeat(92));
  stdout.push("route".padEnd(42) + "th\xE8me ".padEnd(7) + "vue".padEnd(8) + "r\xE9sultat");
  for (const r of rows) {
